...
void ScreenToDark(Game &game, GLFWwindow *window_main)
{
...
//	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
  ...
  GameMap lvl1;
  lvl1.loadFile("test.txt", &lvl1.player_start, spike_anim);
  lvl1.backup = backup;
  GameMap lvl2;
  ...
  //game loop
    ...
    if (game.screen_load)
    {
      if (screenBuffer.k == 0)
      {
        ...
        ScreenToDark(game, window_main);
        screenBuffer.k += 0.01;
        continue;
      }
      else if (screenBuffer.k < 1.01)
      {
        screenBuffer.k += 0.01;
        ScreenToDark(game, window_main);
        continue;
